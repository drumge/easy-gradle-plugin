...
import com.drumge.easy.plugin.api.EasyUtils
import com.drumge.easy.plugin.api.TypeUtils
...
//        Log.i(TAG, "isNeedUnzipJar name: %s", name)
        if (name.contains(KVO_API_ARTIFACT)) {
            return true
        } else if (name.contains(KVO_ANNOTATION_ARTIFACT)) {
...
//            Log.i(TAG, "onAfterJar handleKvo unzipJar: %s", unzipJar)
            return handleKvo(unzipJar)
        }.each { String unzipJar ->
//            Log.i(TAG, "onAfterJar rezipUnzipJarFile unzipJar: %s", unzipJar)
